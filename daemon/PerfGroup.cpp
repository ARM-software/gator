...
bool PerfGroup::createCpuGroup(const uint64_t currTime, Buffer * const buffer)
{
    ...
    mLeaders[PERF_TYPE_HARDWARE] = doAdd(
            currTime,
            buffer,
            schedSwitchKey,
    ...
    if (!(flags & PERF_GROUP_LEADER) && mLeaders[effectiveType] < 0) {
        ...
        if (effectiveType == PERF_TYPE_HARDWARE) {
            ...
            const uint64_t timeout =
                    gSessionData.mSampleRate > 0 && !gSessionData.mIsEBS ? 1000000000UL / gSessionData.mSampleRate :
                                                                           100000000UL;
...
int PerfGroup::prepareCPU(const int cpu, Monitor * const monitor)
{
    ...
    for (int i = 0; i < ARRAY_LENGTH(mKeys); ++i) {
        ...
        if ((mFlags[i] & PERF_GROUP_PER_CPU) && !(mFlags[i] & PERF_GROUP_ALL_CLUSTERS)
                && gSessionData.mSharedData->mClusters[gSessionData.mSharedData->mClusterIds[cpu]] != mClusters[i]) {
            continue;
        }
        ...
        mFds[offset] = sys_perf_event_open(
                &mAttrs[i],
                -1,
                cpu,
                mAttrs[i].pinned ?
                        -1 : mFds[mLeaders[getEffectiveType(mAttrs[i].type, mFlags[i])] * gSessionData.mCores + cpu],
                mAttrs[i].pinned ? 0 : PERF_FLAG_FD_OUTPUT);
        if (mFds[offset] < 0) {
...
static bool readAndSend(const uint64_t currTime, Buffer * const buffer, const int fd, const int keyCount,
...
int PerfGroup::onlineCPU(const uint64_t currTime, const int cpu, const bool enable, Buffer * const buffer)
{
    ...
    if (!mLegacySupport) {
        ...
        for (int i = 0; i < ARRAY_LENGTH(mLeaders); ++i) {
            if (idCounts[i] > 0
                    && !readAndSend(currTime, buffer, mFds[mLeaders[i] * gSessionData.mCores + cpu], idCounts[i],
                                    coreKeys[i])) {
                return 0;
            }
        }
