...
namespace armnn
{
     ...
     * packet to the metadata version from the bytes passed as argument.
     */
    static lib::Optional<std::uint32_t> isMatchedStreamMetadata(Bytes bytes,
                                                                const ByteOrder byteOrder)
    {
        if (bytes.length >= 4) { //Excluding pipe magic message
    ...
    {
        auto matchedStreamMetadata = isMatchedStreamMetadata(bytes, byteOrder);
        if (matchedStreamMetadata.valid()) {
            ...
            if (majorNumber == 1) { //1.0.0 - return decoder for
                return armnn::decodeStreamMetaData(bytes, byteOrder);
            }
        }
