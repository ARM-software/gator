...
namespace armnn
{
    ...
    DecodingStatus PacketDecoder::decodePacket(std::uint32_t type, Bytes payload)
    {
        ...
        switch (type) {
            ...
            {
                //1.x.x
                if (!armnn::decodeAndConsumePeriodicCounterSelectionPkt(payload, byteOrder,
                                                                        consumer)) {
            ...
            {
                //1.x.x
                if (!armnn::decodeAndConsumePerJobCounterSelectionPkt(payload, byteOrder,
                                                                      consumer)) {
            ...
            {
                //1.x.x
                if (!armnn::decodeAndConsumePeriodicCounterCapturePkt(payload, byteOrder,
                                                                      consumer)) {
            ...
            {
                //1.x.x
                if (!armnn::decodeAndConsumePerJobCounterCapturePkt(true, payload, byteOrder,
                                                                    consumer)) {
            ...
            {
                //1.x.x
                if (!armnn::decodeAndConsumePerJobCounterCapturePkt(false, payload, byteOrder,
                                                                    consumer)) {
                ...
                break;
            default:
                logg.logError("Not supported packet type %u", type);
                return DecodingStatus::Failed;
                break;
        }
    ...
    bool PacketDecoder::isValidPacketVersions(std::vector<PacketVersionTable> pktVersionTable)
    {
            ...
            switch (packetType) {
                ...
                case lib::toEnumValue(PacketType::PostPerJobCounterCapturePkt): //
                {
                    ...
                    if (majorVersion != SUPPORTED_PACKET_MAJOR_VERSION[0]) {
                        logg.logError("Unsupported packet version (%u:%u:%u) for packet type (%u)", majorVersion,getBits(packetVersion, 12, 22),getBits(packetVersion, 0, 11),packetType);
                        return false;
                    }
                    validPacket = true;
                }
                    break;
                default:
                    logg.logError("No decoders supported yet for packet type (%u)", packetType);
                    break;
            }
        }
        return validPacket;
