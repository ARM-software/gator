...
#include "GpuTimelineLayerManager.h"
...
namespace {
    ...
    */
    ...
    std::optional<std::string> get_android_settings(const std::string & setting)
    {
        ...
        if (cmd_output.empty() || cmd_output == "null") {
            return std::nullopt;
        }
        return cmd_output;
    }
...
void GpuTimelineLayerManager::deploy_to_package(const std::string & package)
{
    ...
    const auto cmd_result = android::copy_to_pkg_data_dir(package, data_tmp.path(), std::string(LAYER_DRIVER_LIB));

    if (!cmd_result) {
        LOG_WARNING("Failed to copy layer driver to %s data directory. GPU timeline layer will not be activated",
                    package.c_str());
        return;
    }
    LOG_DEBUG("Layer driver copied to %s", cmd_result->c_str());

    orig_enable_gpu_debug_layers = get_android_settings("enable_gpu_debug_layers");
    orig_gpu_debug_app = get_android_settings("gpu_debug_app");
    orig_gpu_debug_layer_app = get_android_settings("gpu_debug_layer_app");
    orig_gpu_debug_layers = get_android_settings("gpu_debug_layers");

    std::string gpu_debug_layers_value = std::string(LAYER_DRIVER_NAME);

    if (orig_gpu_debug_layers) {

        // If gpu_debug_layers doesn't contain the layer, Append the new layer to the existing layers
        if (orig_gpu_debug_layers->find(gpu_debug_layers_value) == std::string::npos) {
            LOG_DEBUG("gpu_debug_layers Android setting contains '%s' for package %s, %s is going to be "
                      "appended to the existing list",
                      orig_gpu_debug_layers->c_str(),
                      package.c_str(),
                      gpu_debug_layers_value.c_str());
            gpu_debug_layers_value = *orig_gpu_debug_layers + ":" + gpu_debug_layers_value;
        }
        else {
            LOG_DEBUG("gpu_debug_layers already contains %s for package %s, not appending again",
                      gpu_debug_layers_value.c_str(),
                      package.c_str());
            gpu_debug_layers_value = *orig_gpu_debug_layers;
        }
    }
    else {
...
void GpuTimelineLayerManager::clean_from_package()
{
    if (layer_deployed) {
        android::remove_from_pkg_data_dir(package, std::string(LAYER_DRIVER_LIB));

        set_android_settings("enable_gpu_debug_layers", orig_enable_gpu_debug_layers.value_or("0"));
        set_android_settings("gpu_debug_app", orig_gpu_debug_app.value_or("''"));
        set_android_settings("gpu_debug_layer_app", orig_gpu_debug_layer_app.value_or("''"));
        set_android_settings("gpu_debug_layers", orig_gpu_debug_layers.value_or("''"));
        layer_deployed = false;
        LOG_DEBUG("Android settings have been reset to their original values");
    }
}
