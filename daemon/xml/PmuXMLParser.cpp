...
static bool matchPMUName(const char * const pmu_name, const char * const test_name)
{
    ...
    // compare suffix
    return strcasecmp(pmu_name + offset + sizeof(UNCORE_PMNC_NAME_WILDCARD) - 1,
                      test_name + test_offset) == 0;
}

static bool parseCpuId(std::set<int> & cpuIds, const char * const cpuIdStr, bool required,
                       const char * const pmuId, const char * const locationStr)
{
    if (cpuIdStr == nullptr) {
        if (required) {
...
{
    std::unique_ptr<mxml_node_t, void (*)(mxml_node_t*)> documentPtr { mxmlLoadString(
            nullptr, xml, MXML_NO_CALLBACK),
                                                                       mxmlDelete };
    ...
    if ((versionStr == nullptr) || (strcmp(versionStr, "2") != 0)) {
        logg.logError("Invalid or missing version string in 'pmus.xml': (%s)", (versionStr != nullptr ? versionStr : "<missing>"));
        return false;
    }

    for (mxml_node_t *node = mxmlFindElement(root, root, TAG_PMU, nullptr, nullptr, MXML_DESCEND);
            node != nullptr;
        ...
        std::set<int> cpuIds;
        {
            if (!parseCpuId(cpuIds, mxmlElementGetAttr(node, ATTR_CPUID), false, id,
                            "cpuid attribute")) {
                return false;
            }
            for (mxml_node_t *childNode = mxmlFindElement(node, node, TAG_CPUID, nullptr, nullptr,
                                                          MXML_DESCEND); childNode != nullptr;
                    childNode = mxmlFindElement(childNode, node, TAG_CPUID, nullptr, nullptr,
                                                MXML_DESCEND)) {
                if (!parseCpuId(cpuIds, mxmlElementGetAttr(childNode, ATTR_ID), true, id,
                                "cpuid.id attribute")) {
                    return false;
                }
            }
        ...
        if ((id == nullptr) || (strlen(id) == 0) || (counterSet == nullptr)
                || (strlen(counterSet) == 0) || cpuIds.empty() || (coreName == nullptr)
                || (strlen(coreName) == 0) || (pmncCounters <= 0)) {
            logg.logError("A pmu from the pmu XML is missing one or more of the required attributes (%s, %s, %s and %s)", ATTR_ID, ATTR_CPUID, ATTR_CORE_NAME, ATTR_PMNC_COUNTERS);
            return false;
        }
    ...
    for (mxml_node_t *node = mxmlFindElement(root, root, TAG_UNCORE_PMU, nullptr, nullptr,
                                             MXML_DESCEND); node != nullptr;
            node = mxmlFindElement(node, root, TAG_UNCORE_PMU, nullptr, nullptr, MXML_DESCEND)) {
        const char * const id = mxmlElementGetAttr(node, ATTR_ID);
        const char * const coreName = mxmlElementGetAttr(node, ATTR_CORE_NAME);
        const char * const counterSetAttr = mxmlElementGetAttr(node, ATTR_COUNTER_SET);
        const char * const counterSet = (counterSetAttr != nullptr ? counterSetAttr : coreName); // uses core name as default
        const char * const pmncCountersStr = mxmlElementGetAttr(node, ATTR_PMNC_COUNTERS);
        int pmncCounters;
        if (!stringToInt(&pmncCounters, pmncCountersStr, 0)) {
            logg.logError("The pmnc_counters for '%s' in pmu XML is not an integer", id);
        ...
        if ((id == nullptr) || (strlen(id) == 0) || (counterSet == nullptr)
                || (strlen(counterSet) == 0) || (coreName == nullptr) || (strlen(coreName) == 0)
                || (pmncCounters == 0)) {
            logg.logError("An uncore_pmu from the pmu XML is missing one or more of the required attributes (%s, %s and %s)", ATTR_ID, ATTR_CORE_NAME, ATTR_PMNC_COUNTERS);
            return false;
        }
        ...
        if (strstr(id, UNCORE_PMNC_NAME_WILDCARD) == nullptr) {
            ...
            if (!matched) {
                logg.logMessage("No matching devices for wildcard <%s %s=\"%s\" %s=\"%s\" %s=\"%s\" %s=\"%s\" %s=\"%d\" />",
                        TAG_UNCORE_PMU,
                        ATTR_CORE_NAME, coreName,
                        ATTR_ID, id,
                        ATTR_COUNTER_SET, counterSet,
