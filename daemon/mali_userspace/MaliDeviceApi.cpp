...
namespace mali_userspace
{
     ...
     * @param shaderCores
     */
    static void logDetectedMaliDevice(const char * maliDevicePath, uint32_t productId, uint32_t major, uint32_t minor, uint32_t frequency, uint32_t l2Slices, uint32_t shaderCores)
    {
        ...
        if (productName != nullptr) {
            formatter << " as Mali-" << productName << " (0x" << std::hex << productId << std::dec << " r" << major << "p" << minor << ")";
            if (frequency > 0) {
            ...
            formatter << " but it is not recognized (id: 0x" << std::hex << productId << std::dec << " r" << major << "p" << minor << ", " << l2Slices << " L2 slices, " << shaderCores
    ...
    namespace ddk_pre_r21
    {
        ...
        public:
            ...
            virtual lib::AutoClosingFd createHwCntReaderFd(size_t bufferCount, uint32_t jmBitmask,
                                                           uint32_t shaderBitmask,
                                                           uint32_t tilerBitmask,
                                                           uint32_t mmuL2Bitmask,
                                                           bool & failedDueToBufferCount) override
            {
                logg.logMessage("MaliDeviceApi: create (%zu, 0x%x, 0x%x, 0x%x, 0x%x)", bufferCount, jmBitmask, shaderBitmask, tilerBitmask, mmuL2Bitmask);
                ...
                if (doMaliIoctl(*devFd, setup_args) != 0) {
                    if (setup_args.header.ret != 0) {
                        logg.logMessage("MaliDeviceApi: Failed sending hwcnt reader ioctl. fd=%i ret = %lu", *devFd, static_cast<unsigned long>(setup_args.header.ret));
                    }
                    else {
        ...
        std::unique_ptr<IMaliDeviceApi> probe(const char * maliDevicePath, lib::AutoClosingFd devFd)
        {
            // get & check API version
            {
                ...
                if (doMaliIoctl(*devFd, version_check) != 0) {
                    ...
                    logg.logMessage("MaliDeviceApi: Unsupported ABI version %u.%u", version_check.major, version_check.minor);
                    return {};
                }
    ...
    namespace ddk_post_r21
    {
        ...
        static inline uint16_t readU16(uint8_t * buffer, int & pos, int size)
        {
            ...
            const uint16_t result = buffer[pos]
        ...
        static inline uint64_t readValue(KBaseGpuPropValueSize value_size, uint8_t * buffer, int & pos, int size)
        {
            switch (value_size)
            {
            case KBaseGpuPropValueSize::U8:  return readU8(buffer, pos, size);
            case KBaseGpuPropValueSize::U16: return readU16(buffer, pos, size);
            case KBaseGpuPropValueSize::U32: return readU32(buffer, pos, size);
            case KBaseGpuPropValueSize::U64: return readU64(buffer, pos, size);
            default: std::terminate(); // impossible
            }
        }
        ...
        public:
            ...
            {
                logDetectedMaliDevice(maliDevicePath, props.product_id, props.major_revision, props.minor_revision, 0, props.num_l2_slices, numberOfShaderCores);
            }
                                                           ...
                                                           bool & failedDueToBufferCount) override
            {
                ...
                const int hwcntReaderFd = lib::ioctl(*devFd, KBASE_IOCTL_HWCNT_READER_SETUP, reinterpret_cast<unsigned long>(&setup_args));
                if (hwcntReaderFd < 0) {
                    logg.logMessage("MaliDeviceApi: Failed sending hwcnt reader ioctl");
                    failedDueToBufferCount = true;
                    return {};
        ...
        std::unique_ptr<IMaliDeviceApi> probe(const char * maliDevicePath, lib::AutoClosingFd devFd)
        {
            ...
            // read the GPU properties
            {
                ...
                // decode the properties data
                {
                    ...
                    return std::unique_ptr<IMaliDeviceApi>{new MaliDeviceApi(maliDevicePath, std::move(devFd), properties)};
                }
            }
    ...
    {
        {
            ...
            if (!devFd) {
                if (errno != ENOENT) {
                    logg.logMessage("MaliDeviceApi: Failed to open mali device '%s' due to '%s'", devMaliPath, strerror(errno));
                }
                return {};
            }
