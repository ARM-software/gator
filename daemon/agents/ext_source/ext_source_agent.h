...
#include "agents/common/timeline_socket_worker.h"
...
namespace agents {
    ...
    public:
        using accepted_message_types = std::tuple<ipc::msg_annotation_send_bytes_t,
                                                  ipc::msg_annotation_close_conn_t,
                                                  ipc::msg_gpu_timeline_configuration_t>;

        using timeline_socket_read_worker_type = timeline_socket_worker_t<ipc_timeline_sink_adapter_t>;
        using annotation_socket_read_worker_type = socket_read_worker_t<ipc_annotations_sink_adapter_t>;

        /**
         * UDS socket name for timeline data. Must agree with the socket name in
         * layer_gpu_timeline/source/device.cpp of the libGPUlayers repo.
         */
        static constexpr std::string_view timeline_socket_name {"\0lglcomms", 9};
        ...
        /**
         * Add both annotation socket listeners as UDS sockets
         * @param parent_name ABSTRACT socket name for parent listener
         * @param data_name ABSTRACT socket name for data listener
         */
        void add_uds_annotation_listeners(std::string_view parent_name, std::string_view data_name)
        {
            ...
            boost::asio::post(strand, [st = shared_from_this(), parent_name, data_name]() {
                if (!st->is_shutdown) {
        ...
        /**
         * Add both annotation socket listeners as TCP sockets
         * @param parent Socket description for parent listener
         * @param data Socket description for data listener
         */
        void add_tcp_annotation_listeners(boost::asio::ip::tcp::endpoint const & parent,
                                          boost::asio::ip::tcp::endpoint const & data)
        {
            // strand is used for synchronizing access to internal structures
            boost::asio::post(strand, [st = shared_from_this(), parent, data]() {
                if (!st->is_shutdown) {
                    st->on_strand_add_agent(
        ...
        /**
         * Add GPU Timeline socket listeners as UDS sockets
         * @param socket_name ABSTRACT socket name
         */
        void add_gpu_timeline_listeners(std::string_view socket_name)
        {
            // strand is used for synchronizing access to internal structures
            boost::asio::post(strand, [st = shared_from_this(), socket_name]() {
                if (!st->is_shutdown) {
                    st->on_strand_add_agent("GPU timeline UDS listener",
                                            make_uds_socket_listener(
                                                [st](auto socket) { st->spawn_gpu_timeline_worker(std::move(socket)); },
                                                st->io_context,
                                                uds_protocol_t::endpoint {socket_name}));
                }
            });
        }
        ...
        void add_all_defaults()
        {
            ...
            // GPU timeline listener is added in co_configure_gpu_timeline()
        }
        ...
        async::continuations::polymorphic_continuation_t<> co_receive_message(ipc::msg_gpu_timeline_configuration_t msg)
        {
            return co_configure_gpu_timeline(msg.header);
        }
    ...
    private:
        ...
        std::map<ipc::annotation_uid_t, std::shared_ptr<annotation_socket_read_worker_type>> annotation_socket_workers;
        std::map<ipc::annotation_uid_t, std::shared_ptr<timeline_socket_read_worker_type>> timeline_socket_workers;
            ...
            ipc::msg_annotation_send_bytes_t message)
        {
            ...
            return start_on(strand)
                 | then([self, message = std::move(message)]() mutable -> polymorphic_continuation_t<> {
                       ...
                       auto worker_it = self->annotation_socket_workers.find(message.header);
                       if (worker_it == self->annotation_socket_workers.end()) {
        ...
        async::continuations::polymorphic_continuation_t<> co_shutdown_workers()
        {
            ...
            return start_on(strand)
                 ...
                 // then close all of the annotation workers
                 | iterate(annotation_socket_workers,
                           [self](auto it) mutable {
                               auto worker = it->second;

                               LOG_TRACE("Closing annotation worker %d (%p)", it->first, worker.get());

                               static_assert(!std::is_const_v<decltype(worker)>);
                               return worker->async_close(use_continuation);
                           })
                 | then([self]() mutable { self->annotation_socket_workers.clear(); })
                 // then close all of the timeline workers
                 | iterate(timeline_socket_workers,
                           [self](auto it) mutable {
                               auto worker = it->second;

                               LOG_TRACE("Closing timeline worker %d (%p)", it->first, worker.get());

                               static_assert(!std::is_const_v<decltype(worker)>);
                               return worker->async_close(use_continuation);
                           })
                 | then([self]() mutable { self->timeline_socket_workers.clear(); })
        ...
        {
            // strand is used for synchronizing access to internal structures
            boost::asio::post(strand, [st = shared_from_this(), socket = std::move(socket)]() mutable {
                if (st->is_shutdown) {
        ...
        /**
         * Called whenever a new annotation connection is accepted, to create a new worker from the socket.
         */
        template<typename Socket>
        void spawn_worker(Socket socket)
        {
            ...
            boost::asio::post(strand, [st = shared_from_this(), socket = std::move(socket)]() mutable {
                if (st->is_shutdown) {
                    LOG_DEBUG("Shutdown: Dropping new inbound annotation connection");
                    return;
                }
                ...
                auto socket_read_worker =
                    annotation_socket_read_worker_type::create(st->io_context,
                                                               ipc_annotations_sink_adapter_t(st->ipc_sink, id),
                                                               make_socket_ref(std::move(socket)));

                // store it
                st->annotation_socket_workers[id] = socket_read_worker;
        ...
        /**
         * Called whenever a new GPU Timeline connection is accepted, to create a new worker from the socket.
         */
        template<typename Socket>
        void spawn_gpu_timeline_worker(Socket socket)
        {
            // strand is used for synchronizing access to internal structures
            boost::asio::post(strand, [st = shared_from_this(), socket = std::move(socket)]() mutable {
                if (st->is_shutdown) {
                    LOG_DEBUG("Shutdown: Dropping new inbound GPU Timeline connection");
                    return;
                }
                ...
                auto socket_read_worker =
                    timeline_socket_read_worker_type::create(st->io_context,
                                                             ipc_timeline_sink_adapter_t(st->ipc_sink, id),
                                                             make_socket_ref(std::move(socket)));

                // store it
                st->timeline_socket_workers[id] = socket_read_worker;
        ...
        /** Close a worker (Annotation or GPU Timeline) given its unique ID */
        async::continuations::polymorphic_continuation_t<> co_close_worker_by_id(ipc::annotation_uid_t id)
        {
            using namespace async::continuations;
            if (is_shutdown) {
                LOG_DEBUG("Ignoring connection close request for ID [%d] since this agent is shutting down "
                          "and all connections will be closed.",
                          id);
                return {};
            }
            ...
            return start_on(strand) | then([self, id]() -> async::continuations::polymorphic_continuation_t<> {
                       auto annotation_worker_it = self->annotation_socket_workers.find(id);
                       bool is_not_annotation_worker = (annotation_worker_it == self->annotation_socket_workers.end());
                       auto timeline_worker_it = self->timeline_socket_workers.find(id);
                       bool is_not_timeline_worker = (timeline_worker_it == self->timeline_socket_workers.end());

                       if (is_not_annotation_worker && is_not_timeline_worker) {
                           LOG_DEBUG("Received close request for non-existent client %d", id);
                           return {};
                       }

                       if (is_not_annotation_worker) {
                           // GPU Timeline worker
                           auto timeline_worker = timeline_worker_it->second;
                           if (!timeline_worker) {
                               LOG_DEBUG("Received close request for non-existent GPU Timeline client %d", id);
                               return {};
                           }

                           // remove from the map
                           self->timeline_socket_workers.erase(timeline_worker_it);

                           // close it
                           return timeline_worker->async_close(async::continuations::use_continuation);
                       }

                       // Annotation worker
                       auto annotation_worker = annotation_worker_it->second;
                       if (!annotation_worker) {
                           LOG_DEBUG("Received close request for non-existent annotation client %d", id);
                           return {};
                       }

                       self->annotation_socket_workers.erase(annotation_worker_it);
                       return annotation_worker->async_close(async::continuations::use_continuation);
                   });
        }

        async::continuations::polymorphic_continuation_t<> co_configure_gpu_timeline(bool enable_gpu_timeline)
        {
            using namespace async::continuations;
            ...
            return start_on(strand) //
                 | then([self, enable_gpu_timeline]() mutable -> polymorphic_continuation_t<> {
                       if (enable_gpu_timeline) {
                           LOG_DEBUG("Listening for timeline data...");
                           self->add_gpu_timeline_listeners(ext_source_agent_t::timeline_socket_name);
                       }
                       else {
                           LOG_DEBUG("NOT listening for timeline data");
                       }
