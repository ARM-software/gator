...
#include "pipe_agents/common/socket_reference.h"
            ...
            socket_ref->with_socket([st = this->shared_from_this()](auto & socket) {
                socket.async_read_some(
                    ...
                    [st](auto ec, auto n_read) mutable {
                        if (ec) {
                            // log it and close the connection
                            LOG_ERROR("(%p) Error occured reading bytes for external connection %d, dropping due to %s",
                                      st.get(),
                                      st->socket_ref->native_handle(),
                                      ec.message().c_str());
                            return st->async_close([st]() { LOG_ERROR("(%p) Was closed", st.get()); });
                        }
        ...
        void do_forward_inward_bytes(std::size_t n_to_write)
        {
            ...
            return ipc_sink.async_send_received_bytes(
                ...
                [st = this->shared_from_this()](auto const & ec, auto msg) mutable {
                    ...
                    if (ec) {
                        // log it and close the connection
                        LOG_ERROR("(%p) Error occured forwarding bytes for external connection %d, dropping due to %s",
                                  st.get(),
                                  st->socket_ref->native_handle(),
                                  ec.message().c_str());
                        return st->async_close([st]() { LOG_ERROR("(%p) Was closed", st.get()); });
